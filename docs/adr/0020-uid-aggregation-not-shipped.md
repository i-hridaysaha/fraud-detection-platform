...
returned no text when fetched, and the same held in this session for both the competition
discussion page and the first-place writeup page: each returned a title and no body.

What was fetched and read this session is an NVIDIA Technical Blog post by Carol McDonald and
...
anything read this session and no claim about it appears in this repo. The argument below does not
